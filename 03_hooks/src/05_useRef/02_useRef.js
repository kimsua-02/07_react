/*
    ...
    3. 애니메이션, 타이머 등 특정 값이 변경 되더라도 리렌더링을 원치 않을 대 저장 용도
    4. DOM 요소에 직접 접급하는 방법을 제공함. 특정 요소 포커스 등
 */
...
const UseRefCounter = () => {
    ...
    // 카운트 증가 버튼 클릭 시 counter 값이 올라가면서 콘솔 창에 useRefCounter 렌더링 됨 ~.. 이 나타남.

    const [count, setCount] = useState(0);

    let variable = 0;
    // React App 화면 창에 ' varialbe : 0 ' 으로 나타남.
    ...
    const increseVarible = () => {
        variable += 1;
        console.log("variable : " + variable);
        // varible 버튼 클릭 시 React App 화면 창에는 값이 올라가지 않음. 콘솔 창에서만 값이 올라가면서 나타남.
    }
    ...
    return (
        <>
            ...
            <button onClick={increseVarible}>variable 증가</button>
